...
class Proveedor(models.Model):
    proveedor = models.CharField(
        max_length=100
    )  # los id se ponen a 100 porque puede incrementar los registros
...
class Cliente(models.Model):
    clienteUsuario = models.OneToOneField(
        Usuario, on_delete=models.CASCADE, related_name="clienteUsuario"
    )
    ...
    direccion = models.TextField(
        null=True, blank=True
    )  # puede existir registos de clientes sin correo
...
class MetodoPago(models.Model):
    ...
    fecha_ultima_actualizacion = models.DateTimeField(
        auto_now=True
    )  # se usa este parametro para poder manejar la fecha exacta de que se actualizó algún campo en el modelo MetodoPago
    # nuevo atributo
...
class Pedido(models.Model):
    ...
    cliente = models.ForeignKey(
        Cliente, on_delete=models.CASCADE, related_name="pedido_cliente"
    )
...
class PiezaMotor(models.Model):
    ...
    pedido = models.ManyToManyField(
        Pedido, through="PiezaMotor_Pedido"
    )  # tabla intermedia entre pedido y piezaMotor
    precio = models.DecimalField(
        max_digits=10, decimal_places=2
    )  # se permite 10 digitos en total de los cuales 2 pueden esta en el punto decimal
    descripcion = models.TextField()
    stock_disponible = models.IntegerField(
        null=True, blank=True
    )  # se permite que este campo este vacio
...
class PiezaMotor_Pedido(models.Model):
    ...
    cantidad = models.IntegerField(
        null=True, blank=True
    )  # se pone estos parametros para evitar el el error que no pueda ser nulo el valor
    precioTotal = models.FloatField(
        default=0.0
    )  # se agrega este parametros para evitar error: Internal error: NOT NULL constraint failed
